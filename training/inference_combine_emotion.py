...
def inference(arg):
    ...
    unet = UNet2DConditionModel.from_pretrained('model/stable-diffusion-v1-5', subfolder="unet")
    unet.to(device)
    ...
    for (emotion_1, emotion_2) in combinations:
        ...
        # pdb.set_trace()
        root_dir1 = f"/data/Users/zyj/EmoGen/data/EmoSet/image/{emotion_1}"
        root_list1 = os.listdir(root_dir1)
        root_dir2 = f"/data/Users/zyj/EmoGen/data/EmoSet/image/{emotion_2}"
        root_list2 = os.listdir(root_dir2)

        index1 = np.random.randint(0, len(root_list1), size=1)
        path1 = os.path.join(root_dir1, root_list1[index1[0]])
        image1 = Image.open(path1)
        img_feat1 = tfm(image1.copy())
        img_feat1 = img_feat1.unsqueeze(0).to(device)

        index2 = np.random.randint(0, len(root_list2), size=1)
        path2 = os.path.join(root_dir2, root_list2[index2[0]])
        image2 = Image.open(path2)
        img_feat2 = tfm(image2.copy())
        img_feat2 = img_feat2.unsqueeze(0).to(device)
        # pdb.set_trace()
        emo_v1, patch_token, visual_query_list1 = encoder(img_feat1, scene_text=None, object_text=None)
        emo_v2, patch_token, visual_query_list2 = encoder(img_feat2, scene_text=None, object_text=None)
        ...
        for _ in range(num_picture):
            ...
            embed_1 = mapper(emo_v1)
            # scene_query, object_query = visual_query_list1[0], visual_query_list1[1]
            ...
            #     attr_rate_scene = attr_coefficient[total_attr[index_scene]][label2idx[emotion_1]].item()
            # if total_attr[index_object] in  attr_coefficient.keys():
            #     attr_rate_object = attr_coefficient[total_attr[index_object]][label2idx[emotion_1]].item()

            # pred_emd1 = embed_1 + attr_rate_scene * pred_emd_scene + attr_rate_object * pred_emd_object 



            token_embeds[token_id] = embed_1
            embed_2 = mapper(emo_v2)

            # scene_query1, object_query1 = visual_query_list2[0], visual_query_list2[1]
        
            # pred_emd_scene  = mapper_scene(scene_query1)
            # pred_emd_object  = mapper_object(object_query1)
            ...
            # pred_emd2 = embed_2 + attr_rate_scene * pred_emd_scene + attr_rate_object * pred_emd_object
...
def generate(cur_dir, device, model, num_fc_layers=1, need_LN=False, need_ReLU=False, need_Dropout=False):
    ...
    parser.add_argument('--num_picture', type=int, default=5)  ##############  TODO
