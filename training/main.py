...
class TextualInversionDataset(Dataset):
    def __init__(
        ...
        if learnable_property != ["all"]:
            ...
            self.data_root = "/data/Users/zyj/EmoGen/data/EmoSet"  # change it into your EmoSet file location
...
def main(args):
    ...
    parameters = list(mapper.parameters()) + list(mapper_scene.parameters()) + list(mapper_object.parameters()) 
    # parameters = list(mapper.parameters()) + list(mapper_scene.parameters()) 
    # parameters = list(mapper.parameters()) + list(mapper_object.parameters()) 
    # parameters = list(mapper.parameters()) + list(CTI.parameters()) 
    ...
    # 
    text_encoder, optimizer, train_dataloader, lr_scheduler, encoder, mapper, model, classifier, mapper_scene, mapper_object = accelerator.prepare(
        text_encoder, optimizer, train_dataloader, lr_scheduler, encoder, mapper, model, classifier, mapper_scene, mapper_object
    )
    ...
    # logger = create_logger(output_dir=args.output_dir, accelerator=accelerator, name=f"{"ViT-L/14"}")
    num = 0
    if hasattr(model, 'module'):
                ...
                with accelerator.accumulate(mapper):
                    with accelerator.accumulate(mapper_scene):
                        with accelerator.accumulate(mapper_object):
                            ...
                            emo_v, patch_token, visual_query_list = encoder(batch["image"], scene_text=None, object_text=None)####1x1024 
                            # pdb.set_trace()
                            ...
                            # atrr_index = index_scene if score_scene[: , index_scene[0]] > score_object[:, index_object[0]] else index_object
                            # 
                            
                            if total_attr[index_scene] in attr_coefficient.keys():
                                attr_rate_scene = attr_coefficient[total_attr[index_scene]][label2idx[batch['emotion'][0]]].item()
                            if total_attr[index_object] in  attr_coefficient.keys():
                                attr_rate_object = attr_coefficient[total_attr[index_object]][label2idx[batch['emotion'][0]]].item()
                            # 
                            # pred_emd = pred_emd_emo + attr_rate_object * pred_emd_object
                            # attr_rate_scene = 0 if attr_rate_scene < 0.1 else attr_rate_scene
                            # attr_rate_object = 0 if attr_rate_object < 0.1 else attr_rate_object
                            
                            pred_emd = pred_emd_emo +   attr_rate_scene * pred_emd_scene +  attr_rate_object * pred_emd_object
                            # pred_emd = pred_emd_emo 

                            # pred_emd = pred_emd_emo +   sce_coefficient(pred_emd_scene) +  obj_coefficient(pred_emd_object)
                            # pred_emd = pred_emd_emo + attr_rate_scene * pred_emd_scene
                            

                            # pred_emd = mapper(CTI(fusion_fea))



                            # Project emo_v into embedding space
                            # pred_emd = mapper(emo_v)#####1x768
                            ...
                            # index_attr = atrr_index[0]
                            # index_attr = torch.tensor([index_attr]).detach().to(score.device)
                            # loss_attr = fun_loss_attr(score, index_attr)
                            # if total_attr[index_attr] in attr_coefficient.keys():
                            #     attr_rate = attr_coefficient[total_attr[index_attr]][label2idx[batch['emotion'][0]]].item()
                            # else:
                            #     attr_rate = 0
                            # else:
                            #     loss_attr = torch.tensor([0.0], requires_grad=True).to(loss_reconstruction.device)
                            ...
                            # loss_forward = (1 - attr_rate)*loss_reconstruction + args.attr_rate * attr_rate * loss_attr 
