...
import sys
from model import *
sys.path.append("/data/Users/zyj/EmoGen/TIP 2026")####
# print(sys.path)
...
def inference(arg, emotion):
    ...
    e_mean = torch.load(f"UniEmo_space/{emotion}_cls_fea_mean.pt")
    e_std = torch.load(f"UniEmo_space/{emotion}_cls_fea_std.pt")
    e_mean_scene = torch.load(f"UniEmo_space/{emotion}_scene_mean.pt")
    e_std_scene = torch.load(f"UniEmo_space/{emotion}_scene_std.pt")
    e_mean_object = torch.load(f"UniEmo_space/{emotion}_object_mean.pt")
    e_std_objecat = torch.load(f"UniEmo_space/{emotion}_object_std.pt")

    normal = torch.distributions.Normal(e_mean, e_std)
    normal_scene = torch.distributions.Normal(e_mean_scene, e_std_scene)
    normal_object = torch.distributions.Normal(e_mean_object, e_std_objecat)
    ...
    checkpoint = torch.load(os.path.join(working_path, "VIT_query.pth"), map_location='cpu')
    ...
    # root_dir = f"/data7/Users/zyj/CVPR2025/EmoGen/data/EmoSet/image/{emotion}"
    ...
    for _ in range(num_picture):
        ...
        e_scene = normal_scene.sample((1,))
        e_object = normal_object.sample((1,))
        ...
        pred_emd_scene  = mapper_scene(e_scene.to(device))
        pred_emd_object  = mapper_object(e_object.to(device))
